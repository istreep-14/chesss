...
function ensureSheet() {
  ...
  // Ensure ECO, Opening URL, and PGN exist (PGN after Opening URL)
  ...
  var hasPgn = headers.indexOf('PGN') !== -1;
  var toAdd = [];
  ...
  // Insert PGN right after Opening URL if missing
  updatedLastCol = sheet.getLastColumn();
  headers = sheet.getRange(1, 1, 1, updatedLastCol).getValues()[0];
  var openingUrlIdx = headers.indexOf('Opening URL');
  if (openingUrlIdx !== -1 && !hasPgn) {
    sheet.insertColumnsAfter(openingUrlIdx + 1, 1);
    sheet.getRange(1, openingUrlIdx + 2).setValue('PGN');
  } else if (!hasPgn) {
    sheet.insertColumnsAfter(updatedLastCol, 1);
    sheet.getRange(1, updatedLastCol + 1).setValue('PGN');
  }
...
function buildRow(details) {
  return [
    ...
    details.clockSecondsIncrement || '', // Clock Seconds_Incriment (per-ply duration incl inc)
...
function normalizeGame(game) {
  ...
  var clockSecondsIncrementStrFinal = moveTimesStrFinal; // same per-ply duration including increment
...
function backfillMovesAndClocks(limit) {
  ...
  var colClockSecondsInc = headerToIndex['Clock Seconds_Incriment'];
  ...
  var clockSecondsIncVals = colClockSecondsInc ? sheet.getRange(2, colClockSecondsInc, numRows, 1).getValues() : null;
  ...
  var pgnVals = colPgn ? sheet.getRange(2, colPgn, numRows, 1).getValues() : null;
  ...
  for (var i = 0, processed = 0; i < numRows && processed < effectiveLimit; i++) {
    ...
    if (pgnVals && String(pgnVals[i][0] || '').trim() === '') pgnVals[i][0] = pgn;

    // Compute derived fields if columns exist
    if (clockSecondsVals || moveTimesVals || baseVals || incVals || clockSecondsIncVals) {
      ...
      if (clockSecondsIncVals) clockSecondsIncVals[i][0] = derived.moveTimesStr || clockSecondsIncVals[i][0];
    }
    processed++;
  }

  // Batch update the columns at once
  ...
  if (colClockSecondsInc && clockSecondsIncVals) sheet.getRange(2, colClockSecondsInc, numRows, 1).setValues(clockSecondsIncVals);
  ...
  if (colPgn && pgnVals) sheet.getRange(2, colPgn, numRows, 1).setValues(pgnVals);
  return processed;
}
...
function backfillEcoAndOpening(limit) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    ...
    if (pgnVals && String(pgnVals[idx][0] || '').trim() === '') { pgnVals[idx][0] = pgn; wrote = true; }
    if (wrote) processed++;
  }
...
function backfillMovesAndClocksInRange(sheet, startRow, numRows) {
  ...
  var clockSecondsIncVals = colClockSecondsInc ? sheet.getRange(startRow, colClockSecondsInc, numRows, 1).getValues() : null;
  ...
  var pgnVals = colPgn ? sheet.getRange(startRow, colPgn, numRows, 1).getValues() : null;

  var processed = 0;
  for (var i = 0; i < numRows; i++) {
    var currentMoves = String(movesSanVals[i][0] || '').trim();
    var currentClocks = String(clocksVals[i][0] || '').trim();
    var needAny = !(currentMoves && currentClocks);
  ...
  if (colClockSecondsInc && clockSecondsIncVals) sheet.getRange(startRow, colClockSecondsInc, numRows, 1).setValues(clockSecondsIncVals);
  ...
  if (colPgn && pgnVals) sheet.getRange(startRow, colPgn, numRows, 1).setValues(pgnVals);
  return processed;
}
...
function backfillEcoAndOpeningInRange(sheet, startRow, numRows) {
  ...
  for (var i = 0; i < numRows; i++) {
    ...
    if (pgnVals && String(pgnVals[i][0] || '').trim() === '') { pgnVals[i][0] = pgn; wrote = true; }
    if (wrote) processed++;
  }
