...
function runConfiguredFetch() {
  ...
  if (cfg.selection.type === 'count') {
    ...
    if (!startYm || !endYm) {
      throw new Error('Range Start/End must be provided for Archives Mode = Range.');
    }
    selected = selected.filter(function(u) {
      var ym = monthFromArchiveUrl_(u);
      if (!ym) return false;
      return compareYm_(startYm, ym) <= 0 && compareYm_(ym, endYm) <= 0;
    });
  }

  if (!selected.length) return;

  var existingUrls = loadExistingUrls(gamesSheet);
  var rowsToAppend = [];

  for (var i = 0; i < selected.length; i++) {
    var monthUrl = selected[i];
    ...
    for (var j = 0; j < monthData.games.length; j++) {
      ...
      if (existingUrls.has(game.url)) continue; // skip duplicates
      var normalized = normalizeGameForUsername_(game, cfg.username);
      if (!normalized || !normalized.isPlayerInGame) continue;
      rowsToAppend.push(buildRow(normalized));
      existingUrls.add(game.url);
    }
    if (rowsToAppend.length > 0) {
      appendRows(gamesSheet, rowsToAppend);
      var startRow = (gamesSheet.getLastRow() - rowsToAppend.length) + 1;
      var newOpts = {
        includeCallback: !!cfg.groups.archiveApi.calculateNew,
        includeMovesClocks: !!cfg.groups.archiveApi.calculateNew,
        includeEcoOpening: !!cfg.groups.archiveApi.calculateNew,
        includeDerived: !!cfg.groups.derived.calculateNew
      };
      processNewRowsGrouped(startRow, rowsToAppend.length, newOpts);
      rowsToAppend.length = 0;
    }
  }

  // Recalculate over the entire sheet per config toggles
  recalcPerConfig_(cfg);
}

/**
 * Runs recalculation tasks across all rows based on the config toggles.
 */
function recalcPerConfig_(cfg) {
  var sheet = ensureSheet();
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) return;
  var count = lastRow - 1;

  if (cfg.groups && cfg.groups.archiveApi && cfg.groups.archiveApi.recalculate) {
    backfillMovesAndClocksInRange(sheet, 2, count);
    backfillEcoAndOpeningInRange(sheet, 2, count);
    backfillCallbackFieldsInRange(sheet, 2, count);
  }
